...
void CalculateResidual(const Frame& current_frame, const Frame &reference_frame, MotionVector mv, int mbx, int mby, std::vector<int> &residualBlock){
    ...
    residualBlock.resize(BLOCK_SIZE * BLOCK_SIZE);
    
    for(int y = 0; y < BLOCK_SIZE; y++){
        for(int x = 0; x < BLOCK_SIZE; x++){
            uint8_t curr_pixel = current_frame.getPixel(current_block_x + x, current_block_y + y);
            uint8_t ref_pixel = reference_frame.getPixel(current_block_x + x + mv.dx, current_block_y + y + mv.dy);
            residualBlock[y * BLOCK_SIZE + x] = static_cast<int>(curr_pixel) - static_cast<int>(ref_pixel); 
        }
    }
}
        ...
        for(int mbx = 0; mbx < mbWidth; mbx++){
            ...
            for(int y = 0; y < BLOCK_SIZE; y++){
                for(int x = 0; x < BLOCK_SIZE; x++){
                    ...
                    uint8_t pred_pixel = reference_frame.getPixel(pred_x, pred_y);

                    int residual_pixel = residualBlock[y * BLOCK_SIZE + x];
                    int final_pixel = static_cast<int>(pred_pixel) + residual_pixel;

                    final_pixel = std::max(0, std::min(255, final_pixel));

                    output_frame.setPixel(global_x, global_y, final_pixel);
                }
            }
        }
    }
